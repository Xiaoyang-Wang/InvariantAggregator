...
class TextDataset(Dataset):
    def __init__(self, data, test_only=False, user_idx=None, file_type=None):
        ...
        # Get all data
        self.user_list, self.user_data, self.user_data_lens, self.user_data_label, self.num_samples = self.load_data(data, self.file_type)
        ...
        if self.test_only:  # combine all data into single array
            ...
            self.features_lens = np.hstack(list(self.user_data_lens.values()))
            ...
            self.features_lens = self.user_data_lens[self.user]
            self.labels = self.user_data_label[self.user]

    def __getitem__(self, idx):
        return self.features[idx].astype(np.float32), self.features_lens[idx], self.labels[idx]
    ...
    def load_data(data, file_type):
        ...
        features_lens = data['user_data_lens']
        labels = data['user_data_label']
        num_samples = data['num_samples']

        return users, features, features_lens, labels, num_samples