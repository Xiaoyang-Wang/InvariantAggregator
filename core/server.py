...
class OptimizationServer(federated.Server):
    def __init__(self, num_clients, model, optimizer, ss_scheduler, data_path, model_path, train_dataloader,
        ...
        # CIFAR-10
        # with open('path_to_msrflute/msrflute/experiments/backdoor_CIFAR/utils/data/SA/southwest_images_new_train.pkl',
        #   'rb') as train_f:
        #     saved_southwest_dataset_train = pickle.load(train_f)

        # with open('path_to_msrflute/msrflute/experiments/backdoor_CIFAR/utils/data/SA/southwest_images_new_test.pkl',
        #         'rb') as test_f:
        #     saved_southwest_dataset_test = pickle.load(test_f)


        # poisoned_data = torch.from_numpy(saved_southwest_dataset_train / 255.0).float().permute(0, 3, 1, 2)

        # sampled_targets_array_train = 9 * np.ones((saved_southwest_dataset_train.shape[0],),
        #                                         dtype=int)  # southwest airplane -> label as truck

        # poisoned_target = torch.from_numpy(sampled_targets_array_train)

        # poisoned_trainset = torch.utils.data.TensorDataset(poisoned_data, poisoned_target)

        # self.p_train_dataloader = torch.utils.data.DataLoader(poisoned_trainset, batch_size=128, shuffle=False)

        # poisoned_data = torch.from_numpy(saved_southwest_dataset_test / 255.0).float().permute(0, 3, 1, 2)

        # sampled_targets_array_test = 9 * np.ones((saved_southwest_dataset_test.shape[0],),
        #                                         dtype=int)  # southwest airplane -> label as truck

        # poisoned_target = torch.from_numpy(sampled_targets_array_test)

        # poisoned_testset = torch.utils.data.TensorDataset(poisoned_data, poisoned_target)

        # self.p_test_dataloader = torch.utils.data.DataLoader(poisoned_testset, batch_size=128, shuffle=False)

        # Model360
        poisoned_data = torch.load('path_to_msrflute/msrflute/experiments/backdoor_model360/utils/data/backdoor_data.pt')
        poisoned_target = torch.load('path_to_msrflute/msrflute/experiments/backdoor_model360/utils/data/backdoor_label.pt')
        poisoned_trainset = torch.utils.data.TensorDataset(poisoned_data, poisoned_target)
        self.p_train_dataloader = torch.utils.data.DataLoader(poisoned_trainset, batch_size=128, shuffle=False)
        self.p_test_dataloader = torch.utils.data.DataLoader(poisoned_trainset, batch_size=128, shuffle=False)

        self.target_train_acc = []
    ...
    def train(self):
        ...
        try:
            ...
            for i in range(self.cur_iter_no, self.max_iteration):
                ...
                # Combine payloads
                self.strategy.set_client_idx(sampled_idx_clients)
                ...
                # test backdoor acc
                target_train_acc = self.test(self.worker_trainer.model.net, self.p_train_dataloader)
                target_test_acc = self.test(self.worker_trainer.model.net, self.p_test_dataloader)

                # for sent-140
                # target_train_acc, target_test_acc = self.test_rnn(self.worker_trainer.model.net)
                self.target_train_acc.append(target_train_acc / 100.0)
                self.target_test_acc.append(target_test_acc / 100.0)

                if len(self.target_train_acc) % 1 == 0:
                    print_rank('target_train_acc: ' + str(self.target_train_acc))
                    print_rank('target_test_acc: ' + str(self.target_test_acc))
    ...
    def test(self, model, test_loader):
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                # print_rank('pred' + str(pred.view(-1).tolist()))
                target = target.view_as(pred)
                correct += pred.eq(target).sum().item()
                # for c in range(0, 10):
                #     correct_per_class[c] += ((pred == target) * (target == c)).float().sum().item()
                #     sample_per_class[c] += (target == c).float().sum().item()
    ...
    def test_rnn(self, model):
        model.eval()
        test_loss = 0
        correct = 0

        data = torch.load('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_train_data.pt')
        with open('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_train_data_lens.txt', 'rb') as file:
            lens = pickle.load(file)
        target = torch.load('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_train_target.pt')
        ...
        train_acc = 100. * correct / data.shape[0]
        test_loss = 0
        correct = 0

        data = torch.load('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_test_data.pt')
        with open('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_test_data_lens.txt', 'rb') as file:
            lens = pickle.load(file)
        target = torch.load('path_to_msrflute/msrflute/experiments/backdoor_twitter/utils/data/backdoor_test_target.pt')
        ...
        test_acc = 100. * correct / data.shape[0]
