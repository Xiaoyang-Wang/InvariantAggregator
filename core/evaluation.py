...
class Evaluation():
    ...
    def run(self, eval_list, req, metric_logger=None):
        ...
        for mode in eval_list:
            ...
            for key,value in self.metrics.items():
                ...
                if isinstance(self.metrics[key]['value'], np.ndarray):
    ...
    def run_distributed_evaluation(self, dataloader, mode):
        ...
        for result in self.process_testvalidate(val_clients, server_data, mode):
            ...
            val_metrics =  {key: {'value':0.0, 'higher_is_better': False} for key in metrics.keys()} if total == 0 else val_metrics
             
            for key in val_metrics:
                if isinstance(metrics[key]['value'], float):
                    val_metrics[key]['value'] += metrics[key]['value']* count
                elif isinstance(metrics[key]['value'], list):
                    if isinstance(val_metrics[key]['value'], float):
                         val_metrics[key]['value'] = [0.0] * len(metrics[key]['value'])
                    for i in range(len(metrics[key]['value'])):
                        val_metrics[key]['value'][i] += metrics[key]['value'][i]* count
