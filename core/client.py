...
def cifar_backdoor():
    ...
    p_data = torch.utils.data.TensorDataset(poisoned_data, poisoned_target)

    return p_data

def sent140_backdoor():
    ...
    return {'x': data, 'lens': lens, 'y': target}

def model360_backdoor():
...
# p_data = cifar_backdoor()
# p_data = sent140_backdoor()
p_data = model360_backdoor()
...
class Client:
    ...
    def get_data(client_id, dataloader):
        ...
        if isinstance(client_id, int):
            ...
            if 'user_data_label' in _data_dict:  # supervised problem
                ...
                if 'user_data_lens' in _data_dict:
                    lens = _data_dict['user_data_lens'][user_name]
                    if _file_ext == 'hdf5':  # transforms HDF5 Dataset into Numpy array
                        lens = lens[()]
                    
                    return edict({'users': [user_name],
                            'user_data': {user_name: user_data},
                            'user_data_lens': {user_name: lens},
        ...
        # During validation and test, client_id might be a list of integers
            ...
            if 'user_data_label' in _data_dict and 'user_data_lens' in _data_dict:
                users_dict = {'users': [], 'num_samples': [], 'user_data': {}, 'user_data_lens': {}, 'user_data_label': {}}
            elif 'user_data_label' in _data_dict:
            ...
            for client in client_id:
                ...
                if 'user_data_lens' in _data_dict:
                    lens = dataloader.dataset.user_data_lens[user_name]
                    if _file_ext == 'hdf5':
                        lens = lens[()]
                    users_dict['user_data_lens'][user_name] = lens
    ...
    def process_round(client_data, server_data, model, data_path, eps=1e-7):
        ...
        if 'user_data_label' in data_strct.keys() and 'user_data_lens' in data_strct.keys():
            input_strct = edict({
                'users': [user],
                'user_data': {user: data_strct['user_data'][user]},
                'user_data_lens': {user: data_strct['user_data_lens'][user]},
                'num_samples': [data_strct['num_samples'][0]],
                'user_data_label': {user: data_strct['user_data_label'][user]}
            })
        elif 'user_data_label' in data_strct.keys():  # supervised case
            input_strct = edict({
                'users': [user],
                'user_data': {user: data_strct['user_data'][user]},
                'num_samples': [data_strct['num_samples'][0]],
